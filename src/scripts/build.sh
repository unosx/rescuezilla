...
cp "$BASEDIR/src/scripts/build-chroot-steps-part1.bash" "$BASEDIR/src/scripts/build-chroot-steps-part2.bash" "$BASEDIR/src/scripts/common.bash" "$BASEDIR/src/scripts/pkgs.list"  chroot
...
chroot chroot/ /bin/bash -c "IS_INTEGRATION_TEST=$IS_INTEGRATION_TEST ARCH=$ARCH CODENAME=$CODENAME /build-chroot-steps-part1.bash"
if [[ $? -ne 0 ]]; then
...
echo "build-chroot-steps-part1 done"
...
# Enter chroot again
cd "$BUILD_DIRECTORY"
chroot chroot/ /bin/bash /build-chroot-steps-part2.bash
if [[ $? -ne 0 ]]; then
...
mksquashfs chroot image/casper/filesystem.squashfs -comp zstd -b 1M -Xcompression-level "${COMPRESSION_LEVEL}" -e /sys
